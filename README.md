...
Build the core-security-starter library
Install it to your local Maven repository
Build the sample-application with the starter as a dependency
...
Public Access: Health endpoint accessible without auth
Successful Authentication: Valid credentials return JWT
Failed Authentication: Invalid credentials return 401
Token Validation: Valid token grants access
Missing Token: Returns 401 Unauthorized
Role-Based Access: User role denied from admin endpoint (403)
Admin Access: Admin role can access admin endpoints
Test Execution
mvn test -Dtest=SecurityIntegrationTest
Design Decisions & Trade-offs
1. Spring Boot Starter Pattern
Decision: Package security as a reusable starter Rationale:
...
Decision: Use JWT tokens instead of session-based auth Rationale:
...
Decision: Use Spring Boot 3.x AutoConfiguration.imports Rationale:
...
Decision: Use BCrypt with default strength (10) Rationale:
...
Decision: Enable @PreAuthorize annotations Rationale:
...
Decision: Consistent JSON error format across all endpoints Rationale:
...
Decision: Use H2 for sample application Rationale:
...
Role-Based Access Control: URL and method-level authorization
Secure Error Handling: No sensitive information in error messages
Request Logging: Audit trail of authenticated requests
Token Expiration: Configurable JWT expiration
Separation of Concerns: Security logic isolated in starter
Technology Stack
...
This assessment focuses on application-level security architecture and clean modular design. Infrastructure-level concerns such as TLS termination, rate limiting, centralized logging, and API gateway responsibilities are intentionally documented but not implemented, as they are typically handled outside the application layer in real-world production environments
